...
function mk_crutch()
   ...
   assert(new_anim_sprite(eid))
   components.anim_sprite[eid].frames = {9}
   components.anim_sprite[eid].timer = 60
   components.anim_sprite[eid].curr_frame = 1
   components.anim_sprite[eid].timer_reset_val = 60
   components.anim_sprite[eid].on_motion = false 
   components.anim_sprite[eid].loop = true
   ...
   assert(new_affects_squeak(eid))
   components.affects_squeak[eid].val = -0.65
   components.affects_squeak[eid].radius = 64

   return add_component(eid, "is_crutch", true)
end

function update_crutch_position_system()
    for sid, val in pairs(components.is_crutch) do
        local as = components.affects_squeak[sid]
        if as.val > 0.0 then
            as.val = as.val - (0.60/60)
        elseif as.val < 0.0 then
